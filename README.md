...
### 3. SkyrimNet (Chatterbox / Zonos Client)

SkyrimNet uses the Gradio 4 protocol to upload NPC voice samples and trigger zero-shot TTS with inline emotion tags:

- **TTS Engine**: `Chatterbox` (or `Zonos`)
- **Server URL**: `http://192.168.0.253:8080` (or `http://nina.local:8080`)
- **Supported Gradio Endpoints**:
  - `POST /gradio_api/upload`: Multi-part voice sample upload from SkyrimNet
  - `POST /gradio_api/call/generate_audio`: TTS generation request with emotion parsing
  - `GET /gradio_api/call/generate_audio/{event_id}`: Server-Sent Events (SSE) streaming
  - `GET /gradio_api/file={path}`: High-speed WAV file retrieval
- **Supported SkyrimNet / XTTS Endpoints**:
  - `GET /speakers`, `GET /speakers_list`, `GET /get_speakers`
  - `GET /languages`

All emotion tags (`[whisper]`, `[laughter]`, `[angry]`, `[sad]`, etc.) generated by Skyrim LLMs are parsed on the fly and mapped directly to Zonos 2 emotion sliders!
